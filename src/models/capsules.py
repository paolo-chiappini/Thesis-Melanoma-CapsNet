...
class PrimaryCapsules(nn.Module):
    def __init__(
        ...
        self._caps_channels = int(
            output_channels / caps_dim
        )  # number of channels for each capsule

        assert self._caps_channels > 0, "Number of capsules must be greater than 0"
    ...
    def forward(self, x):
        ...
        out = out.view(
            out.size(0), self._caps_channels, out.size(2), out.size(3), self.caps_dim
        )
